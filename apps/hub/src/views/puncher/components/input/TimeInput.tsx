...
function TimeInput(props: { tabIndex?: number, className?: ClassValue, value: string, onSubmit?: (value: string) => void, disabled?: boolean }) {
  ...
  useEffect(() => {
      ...
      setInput(p => {
        ...
        if (p.includes('A') || p.includes('P')) {
          ...
          switch (key) {
            ...
            case '8': inp += '8'; break;
            case '9': inp += '9'; break;
            case 'a': if (inp.length > 0) inp += 'A'; break;
            case 'p': if (inp.length > 0) inp += 'P'; break;
          }
        }
        ...
        if (key === 'Backspace' && inp.includes('M')) { return '' } else { inp = inp.substring(0, inp.length - 1) }
    ...
    current && ref.current.addEventListener('keydown', (e) => InputHandle(e.key))

    return () => {
      current?.removeEventListener('keydown', (e) => InputHandle(e.key))
    }
  }, [validate, disabled])
  ...
  useEffect(() => {
    ...
    function setFocusState() { current?.removeEventListener('focusin', setFocusState); }
    if (current) {
  ...
  return (
    <div tabIndex={!props.disabled ? props.tabIndex ?? 0 : undefined} className={
      cn('border bg-white rounded flex *:flex-1 justify-evenly px-2 py-1 w-fit gap-1 select-none hover:cursor-text font font-mono ',
        {
