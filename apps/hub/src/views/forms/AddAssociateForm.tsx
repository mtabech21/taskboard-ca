...
export function AddAssociateForm(props: { account: ManagerAccount }) {
  ...
  return (
    <div className="w-full justify-center flex select-none overflow-scroll flex-col gap-6">
      ...
      <div>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(form.handler)} className="flex flex-col gap-5 text-left">
            ...
            <Button disabled={!confirmed} className="m-5" onClick={() => { console.log(form.getValues()) }}>Create Associate</Button>
          </form>
        </Form>
      </div>
    </div >
...
export function BranchSelector(props: { branches: Branch[], set_value?: (branch?: Branch) => void }) {
  ...
  useEffect(() => { if (props.set_value) props.set_value(selected_branch); console.log(props) }, [selected_branch, props])
